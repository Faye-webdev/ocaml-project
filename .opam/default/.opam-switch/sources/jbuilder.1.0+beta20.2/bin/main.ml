...
let subst =
  ...
  let man =
    ...
    ; `P {|This command is only meant to be called when a user pins a package to
           its development version. Especially it replaces $(b,1.0+beta20.2) strings
    ...
    ; var "VERSION_NUM" "same as $(b,1.0+beta20.2) but with a potential leading \
                         'v' or 'V' dropped"
    ...
    ; `P {|Note that the expansion of $(b,jbuilder) is guessed using the following
...
let default =
  ...
  , Term.info "jbuilder" ~doc ~version:"1.0+beta20.2"
      ~man:
        [ `S "DESCRIPTION"
